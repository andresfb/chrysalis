# Docker file customized from https://github.com/llaumgui/docker-images/tree/master/php-fpm/7.3
#
...
RUN echo -n "With Bash command:          " ; if [[ "${DOCKER_ENABLE_BASH}" = "on" ]] ;        then echo "Yes"; else echo "No" ; fi && \
    echo -n "With apcu support:          " ; if [[ "${DOCKER_PHP_ENABLE_APCU}" = "on" ]] ;        then echo "Yes"; else echo "No" ; fi && \
...
# Enable Bash
RUN if [ "${DOCKER_ENABLE_BASH}" != "off" ]; then \
      apk add --update --no-cache \
        bash; \
    else \
      echo "Skip Bash command"; \
    fi
...
# --------------------------------------------- Conditional tools installations
# Install composer.
...
RUN sed -i "s/listen = 127.0.0.1:9000/listen = 127.0.0.1:9001/g" /usr/local/etc/php-fpm.d/www.conf
...
RUN if [ "${DOCKER_USER_GID}" != "false" ] && [ "${DOCKER_USER_UID}" = "false" ]; then \
      ...
      groupadd -g ${DOCKER_USER_GID} www && \
      useradd -u ${DOCKER_USER_UID} -ms /bin/bash -g www www; \      
    else \
